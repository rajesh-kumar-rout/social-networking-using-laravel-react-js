...
export default function CommentBox({ postId }) {
    ...
    async function handleAddComment(event) {
        ...
        const { data } = await axios.post(`/posts/${postId}/comments`, payload)
    ...
    return (
        <div className="border-t-2 border-gray-300">
            {isLoading ? (
                <div className="py-4">
                    <div className="h-10 w-10 rounded-full mx-auto border-4 border-teal-600 border-b-transparent animate-spin"></div>
                </div>
            ) : (
                <>
