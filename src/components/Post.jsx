...
console.log(post);
    return (
        <div className="bg-white border-2 border-x-0 sm:border-x-2 border-gray-300 sm:rounded-md mx-auto w-full sm:w-[600px]">
            ...
            {post.description && <div className="text-sm text-gray-600 px-3">{post.description}</div>}

            {post.imageUrl && (
                <img src={postImgUrl(post.imageUrl)} className="w-full object-cover mt-3" />
            )}
            ...
            <div className="p-3 flex justify-between">
                <div className="flex items-center gap-3">
                    <button
                        ...
                        onClick={() => onToggleLike(post.id)}
                    >
                        {post.isLiked == 1 ? <FaHeart size={24} fill="fill-pink-600" /> : <FaRegHeart size={24} />}
                    </button>
